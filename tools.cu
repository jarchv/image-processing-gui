    ...
    int& depth)
{
    ...
    */
    ...
    if (depth < 24){
        ...
        unsigned char* dataEnc = new unsigned char[sizeInput];       
        
        fread(dataEnc, sizeof(unsigned char), sizeInput, f);
        fclose(f);    
        ...
        for (int i = 0; i < sizeInput; i++)
        {
            if (depth == 1){
                ...
                */
                ...
                data[i*3    ] = ColorTable[indColor1*4];
                data[i*3 + 1] = ColorTable[indColor1*4+1];
                data[i*3 + 2] = ColorTable[indColor1*4+2];
